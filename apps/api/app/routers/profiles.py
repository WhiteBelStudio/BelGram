...
async def _mutual_group_count(db: AsyncSession, viewer_id: int, target_id: int) -> int:
    ...
    stmt = (
        select(func.count(func.distinct(mine.group_id)))
        .join(theirs, theirs.group_id == mine.group_id)
...
async def _mutual_community_count(db: AsyncSession, viewer_id: int, target_id: int) -> int:
    ...
    stmt = (
        select(func.count(func.distinct(mine.community_id)))
        .join(theirs, theirs.community_id == mine.community_id)
        .where(mine.user_id == viewer_id, theirs.user_id == target_id)
    )
    return int(await db.scalar(stmt) or 0)
...
async def _profile(db: AsyncSession, viewer: User, target: User) -> ProfilePublic:
    ...
    return ProfilePublic(
        ...
        avatar_url=target.avatar_url,
        bio=target.bio,
        status=target.status if target.show_status else None,
        is_verified=target.is_verified,
        is_online=target.is_online,
        last_seen_at=target.last_seen_at if target.show_last_seen else None,
        mutual_groups=await _mutual_group_count(db, viewer.id, target.id),
...
async def get_profile(
    ...
    if await _is_blocked(db, target.id, user.id):
        raise HTTPException(status_code=404, detail="User not found")
...
async def block_user(
    ...
    if target is None:
        raise HTTPException(status_code=404, detail="User not found")
    if not await _is_blocked(db, user.id, target.id):
